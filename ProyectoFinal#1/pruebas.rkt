...
;; ============================================================
;; Parser + unparse  Mauricio Alejandro Rojas
;; ============================================================
...
(define entrega-1
  (test-suite
   ...
   ;; ---- Ejemplo #1
   (test-case "parser: Ejemplo 1 factorial completo"
     (check-not-exn
      (lambda ()
        (scan&parse
         "letrec fact(n) = if ==(n, 0) then 1 else *(n, apply fact(-(n, 1))) end
   ...
   ;; ---- Ejemplo #3
   (test-case "parser: Ejemplo 3 árbol binario con datatype"
     (check-not-exn
      (lambda ()
        (scan&parse
         "datatype Tree = Leaf(value) | Node(left, right)
