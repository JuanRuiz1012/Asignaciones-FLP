...
(require "map-datatypes.rkt")

(define test-datatypes
  (test-suite
   "Tests - Datatypes"
   (test-case "empty-map es map válido"
     (check-true (map? (empty-map))))
   ...
   (test-case "insert actualiza clave existente"
     (check-equal? (search (insert (insert (empty-map) 5 'hello) 5 'nuevo) 5) 'nuevo))
   ...
   (test-case "delete deja map vacío"
     (check-true (map? (delete (insert (empty-map) 5 'hello) 5))))))

(display "=== Tests Datatypes ===") (newline)
(run-tests test-datatypes)
